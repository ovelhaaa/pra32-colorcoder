...
void PRA32_U2_UI_RenderST7789_draw(const PRA32_U2_UI_RenderFrame& frame) {
  ...
  if ((dirty & PRA32_U2_UI_RenderDirty_Header) != 0) {
    ...
    print_truncated_text(frame.page_name, sizeof(frame.page_name), chars_that_fit(HEADER_PAGE_W));
  ...
  if ((dirty & PRA32_U2_UI_RenderDirty_Footer) != 0) {
    ...
    if (!status_fits_header(frame)) {
      g_st7789.setTextSize(1);
      g_st7789.setTextColor(COLOR_FOOTER_TEXT);
      g_st7789.setCursor(196, 66);
      print_truncated_text(frame.status_text, sizeof(frame.status_text), chars_that_fit(DISPLAY_WIDTH - 196));
    }
  }
